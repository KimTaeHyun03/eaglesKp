...
let Ra = () => {
  ...
  let update = async () => {
    try {
      ...
      let response = await axios.post('http://localhost:3030/api/dataUpdate', {
      ...
      if (response.status === 200) {
        ...
        // 데이터를 다시 가져옴
        const updatedRoles = await axios.get('http://localhost:3030/api/raGet');
  ...
  useEffect(() => {
    const fetchRoles = async () => {
      try {
        const response = await axios.get('http://localhost:3030/api/raGet');
