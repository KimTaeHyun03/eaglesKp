...
let Cook = () => {
  ...
  useEffect(() => {
    const fetchRoles = async () => {
      try {
        const response = await axios.get('http://localhost:3030/api/cookGet');
  ...
  const update = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:3030/api/cookUpdate', {
        sendIndex: index,
      ...
      if (response.status === 200) {
        alert('업데이트 성공');
        // 데이터를 다시 가져옴
        const updatedRoles = await axios.get('http://localhost:3030/api/cookGet');
